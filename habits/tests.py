...
class HabitCreateAPITestCase(APITestCase):
    ...
    def test_create_habit_with_valid_reward(self):
        ...
        self.assertIsNone(created_habit.associated_habit) # Убедимся, что связанной привычки нет
...
class HabitUpdateAPITestCase(APITestCase):
    ...
    def test_update_habit_authenticated_other_user_permission_denied(self):
        ...
        self.habit.refresh_from_db()
        self.assertEqual(self.habit.action, original_action) # Проверяем, что привычка не изменилась
    ...
    def test_update_non_existent_habit(self):
        ...
        self.client.force_authenticate(user=self.owner)
        non_existent_url = reverse('habits:habit_update', kwargs={'pk': 9999}) # Несуществующий PK
        updated_data = {
...
class HabitRetrieveAPITestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.public_habit_url = reverse('habits:habit_detail', kwargs={'pk': self.public_habit.pk})
        self.private_habit_url = reverse('habits:habit_detail', kwargs={'pk': self.private_habit.pk})
    ...
    def test_retrieve_private_habit_other_user_permission_denied(self):
            ...
            response = self.client.get(self.private_habit_url, format='json')
            self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
            self.assertIn("У Вас нет прав просматривать информацию об этой привычке.", response.data['detail'])
    ...
    def test_retrieve_non_existent_habit_fails(self):
        ...
        self.client.force_authenticate(user=self.owner) # Аутентификация не влияет на 404
...
class HabitDestroyAPIViewTest(APITestCase):
    def setUp(self):
        ...
        self.user = User.objects.create(
            email='user@example.com', password='123qwe'
        )
        ...
        self.url_owner_habit = reverse('habits:habit_delete', kwargs={'pk': self.habit_by_owner.pk})
        self.url_other_habit = reverse('habits:habit_delete', kwargs={'pk': self.habit_by_other.pk})
    ...
    def test_owner_deactivates_own_habit(self):
        ...
        self.assertTrue(self.habit_by_owner.is_active) # Проверяем, что изначально активна
    ...
    def test_other_user_cannot_delete_other_habit(self):
        ...
        """
        self.client.force_authenticate(user=self.other_user) # other_user пытается удалить habit_by_owner
        ...
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(response.data['detail'], "У вас нет прав на удаление этой привычки.")
    ...
    def test_unauthenticated_user_cannot_delete_habit(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        self.assertEqual(Habit.objects.count(), initial_habit_count) # Привычка не должна быть удалена

    def test_delete_non_existent_habit(self):
        """
        Тест: Попытка удалить несуществующую привычку должна вернуть 404.
        """
        self.client.force_authenticate(user=self.user) # Любой авторизованный пользователь
...
class HabitListAPIViewTest(APITestCase):
    def setUp(self):
        ...
        self.owner_active_habits_count = Habit.objects.filter(owner=self.owner_user, is_active=True).count() # 8
        self.other_active_habits_count = Habit.objects.filter(owner=self.other_user, is_active=True).count() # 1
    ...
    def test_owner_sees_only_own_active_habits(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['count'], self.owner_active_habits_count) # Должно быть 2
        ...
        expected_ids = set(Habit.objects.filter(
            owner=self.owner_user, is_active=True
        ).order_by('pk')[:CustomPaginator.page_size].values_list('id', flat=True))
        ...
        self.assertNotIn(self.other_habit_active.pk, retrieved_ids) # Чужая не должна быть
    ...
    def test_other_authenticated_user_sees_no_habits(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['count'], self.other_active_habits_count) # Должна быть 1 (owner_user не видит привычки other_user)
        self.assertEqual(len(response.data['results']), self.other_active_habits_count) # 1, т.к. other_user видит только свои
    ...
    def test_pagination_for_owner(self):
        ...
        self.assertEqual(response_page1.data['count'], self.owner_active_habits_count) # Всего 8
...
class PublicHabitListAPIViewTest(APITestCase):
    def setUp(self):
        ...
        self.total_public_habits_count = Habit.objects.filter(is_public=True).count() # Должно быть 8
        self.expected_first_page_ids = set(
            Habit.objects.filter(is_public=True).order_by('pk')[:CustomPaginator.page_size].values_list('id', flat=True)
        )
...
#
    ...
    def test_only_public_habits_are_returned(self):
        ...
        """
        response = self.unauthenticated_client.get(self.url) # Можно использовать любой клиент
    ...
    def test_pagination_works_correctly(self):
        ...
        self.assertEqual(response_page1.status_code, status.HTTP_200_OK)
        self.assertEqual(response_page1.data['count'], self.total_public_habits_count) # Всего 8
        self.assertEqual(len(response_page1.data['results']), CustomPaginator.page_size) # 5 на первой странице
        self.assertIsNotNone(response_page1.data['next']) # Должна быть следующая страница
        ...
        self.assertEqual(len(response_page2.data['results']), self.total_public_habits_count - CustomPaginator.page_size) # 8 - 5 = 3
        ...
        expected_total_ids = set(Habit.objects.filter(is_public=True).values_list('id', flat=True))
        self.assertEqual(all_retrieved_ids, expected_total_ids)

        # Проверяем, что все полученные привычки публичные
        for habit_data in response_page1.data['results'] + response_page2.data['results']:
            self.assertTrue(habit_data['is_public'])
